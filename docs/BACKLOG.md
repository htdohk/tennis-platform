...
## M14.5：C 端首页与预订/招募页面重组（M14 后处理）

### 首页改版
- [ ] 首页两个主按钮：「立即预订」和「立即招募」（替换现有布局）
- [ ] 首页下方展示两个列表：
  - 生效中的预订（状态 PENDING_CONFIRM / CONFIRMED）
  - 生效中的招募（状态 RECRUITING）

### 预订页面（/booking）
- [ ] 顶部 tab：「预订场地」（默认）/ 「我的预订」
- [ ] 「我的预订」列表：先显示生效订单，再接续失效订单（COMPLETED/CANCELLED）

### 招募页面（/recruits）
- [ ] 顶部 tab：「招募广场」（默认）/ 「我的招募」
- [ ] 「我的招募」列表：先显示生效招募（RECRUITING/CONFIRMED），再接续失效（EXPIRED/CANCELLED）
- [ ] 移除当前 /me 页面的「我的招募」独立入口（合并到招募页 tab）
